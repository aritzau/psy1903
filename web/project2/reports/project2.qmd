...
theme_2 <- theme_minimal(base_size = 14) +
  theme(
...
#| label: fig-dscore-hist
#| fig-cap: "Histogram of IAT D-scores across all participants."
d_mean <- mean(participant_wide$d_score, na.rm = TRUE)
d_sd <- sd(participant_wide$d_score, na.rm = TRUE)
d_mean <- mean(participant_wide$d_score, na.rm = TRUE)
d_sd   <- sd(participant_wide$d_score,  na.rm = TRUE)

dscore_hist <- ggplot(participant_wide, aes(x = d_score)) +
  geom_histogram(binwidth = 0.1, fill = "gray", color = "black") +
  
  geom_vline(xintercept = d_mean, color = "red", linetype = "solid", size = 1.0) +
  
  geom_vline(xintercept = d_mean + d_sd, color = "blue", linetype = "dashed", size = 1) +
  geom_vline(xintercept = d_mean - d_sd, color = "blue", linetype = "dashed", size = 1) +
  labs(
    title = "Distribution of IAT D-scores",
    x     = "D-score",
    y     = "Count"
  ) 
...
print(dscore_hist)
```
#| fig-cap: "Histogram of IAT D-scores across participants."

```{r}
  ## (2) bar plot of mean RTs. Include standard error bars.
#| label: fig-barplot-ggplot
#| fig-cap: "Mean reaction times for congruent and incongruent trials with standard deviation error bars. Congruent trials were significantly faster than incongruent trials."

rt_means <- data.frame(
  condition = c("Congruent", "Incongruent"),
  mean      = c(mean(participant_wide$congruent_mean, na.rm=TRUE),
                mean(participant_wide$incongruent_mean, na.rm=TRUE)),
  se        = c(sd(participant_wide$congruent_mean, na.rm=TRUE) / sqrt(sum(!is.na(participant_wide$congruent_mean))),
                sd(participant_wide$incongruent_mean, na.rm=TRUE) / sqrt(sum(!is.na(participant_wide$incongruent_mean))))
)

###Barplot:

#| label: fig-meanrt-bar
#| fig-cap: "Mean reaction times for congruent and incongruent blocks, with SE error bars."

meanrt_bar <- ggplot(rt_means, aes(x = condition, y = mean)) +
  geom_col(fill = "blue") +
...
#| label: fig-correlation-ggplot
#| fig-cap: "Scatterplot showing the relationship between D-scores and anxiety scores with linear regression line and 95% confidence interval."
cor_test <- cor.test(participant_wide$d_score, participant_wide$anxiety_score)

anx_corr_plot <- ggplot(participant_wide, aes(x = d_score, y = anxiety_score)) +
  geom_point(color = "blue") +
  geom_smooth(method = "lm", se = TRUE, color = "red") +
  labs(
    title = "IAT D-score vs. Anxiety Score",
    x     = "D-score",
    y     = "Anxiety Score"
  ) +
  theme_2
...
print(anx_corr_plot)
```
#| fig-cap: "Scatterplot showing the relationship between D-scores and anxiety scores. Clear showing of linear regression line. 95% confidence interval."
...
ggsave("histogram_dscores.png", dscore_hist, 
       path = here::here("outputs", "plots"), 
       width = 8, height = 6, dpi = 300)
ggsave("barplot_mean_rt.png", meanrt_bar, 
       path = here::here("outputs", "plots"), 
       width = 8, height = 6, dpi = 300)
ggsave("correlation_plot.png", anx_corr_plot, 
       path = here::here("outputs", "plots"), 
       width = 8, height = 6, dpi = 300)
#(used ai with saving, wasn't sure on the exact format.)
```
